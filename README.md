...
- The server checks projects daily at 03:00 JST. It asks Workers AI to create a consolidated checkpoint when a project has at least 20 unconsolidated records, or when it has unconsolidated records and three days have elapsed since the oldest pending record or last successful checkpoint. On a project's first due run, the server automatically drains the entire existing backlog in successive bounded AI batches, so old records do not require a manual backfill command. Original records are retained and linked from the checkpoint. Older checkpoints are superseded, and changing or forgetting a source invalidates the checkpoint that depended on it.
- Use the `consolidate_memory` MCP tool to consolidate the current workspace immediately without waiting for the daily check. Pass `projectId` to target a known provenance ID, or `sourceFolder` when the MCP client does not provide exactly one workspace root. Manual execution still requires at least one unconsolidated source record.
...
Replace an installed 1.x `cloudflare-supermemory` plugin with `kagayoi-memory`. During this transition, the client retains compatibility readers for `~/.codex/supermemory.json` and the `SUPERMEMORY_*` and `CLOUDFLARE_MEMORY_*` environment variables. Run `scripts/setup-client.ps1` to migrate an existing connection to `~/.codex/kagayoi-memory.json`, then use the `KAGAYOI_MEMORY_*` variables and `kagayoi-memory` plugin name for normal operation. Existing document IDs, legacy spaces, deterministic capture IDs, and persisted `sm_*` metadata remain valid and are not rewritten merely because the product was renamed.
